...
tree_dir <- "/home/jaman_umass_edu/jaman/Myo2_phylogeny/results/sup_figure_run_4/tree_inference_result"
contree_file_path <- list.files(tree_dir, pattern = "\\.contree$", full.names = TRUE)
tree <- read.tree(contree_file_path)
...
species_groups <- list(
  ...
  Plantae = c("Atha", "Acoe", "Sbic", "Smoe", "Ppat", "Crei", "Vcar", "Esil", "Ngad",
              "Ptri", "Tpse", "Madagascaria"),
  SAR = c("Tgon", "Pfal", "Ptet", "Tthe", "Pmar", "Bnat", "Pinf", "Colponemidia"),
  Metamonada = c("Tvag", "Glam"),
...
annotated_plot <- base_plot %<+% tip_data +
  ...
  geom_cladelab(data = clade_annotation_data,
                ...
                offset = -0.55, fontsize = 2, align = FALSE, barsize = 0.3) +
...
final_plot <- annotated_plot +
  ...
  annotate("rect",
           xmin = legend_x, xmax = legend_x + legend_width,
           ymin = legend_y - legend_height, ymax = legend_y,
