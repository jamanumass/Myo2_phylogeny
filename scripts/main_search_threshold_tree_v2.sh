...
if [ "$run_step_1" = true ]; then
    ...
    job1=$(sbatch --cpus-per-task=$((NUM_CORES + 1)) --export=ALL,shared_variables_file="${shared_variables_file}" -o "$job1_out" -e "$job1_err" "$HMMER_script")    job1_id=$(echo "$job1" | awk '{print $4}')
...
if [ "$run_step_2" = true ]; then
    ...
    job2=$(sbatch --dependency=afterok:$job1_id --cpus-per-task=$((NUM_CORES + 1)) --export=ALL,shared_variables_file="${shared_variables_file}" -o "$job2_out" -e "$job2_err" "$extract_single_domain_script")    job2_id=$(echo "$job2" | awk '{print $4}')
...
if [ -n "$dependency" ]; then
    job3=$(sbatch $dependency --cpus-per-task=$((NUM_CORES + 1)) --export=ALL,shared_variables_file="${shared_variables_file}" -o "$job3_out" -e "$job3_err" "$tree_inference_script")
else
    job3=$(sbatch --cpus-per-task=$((NUM_CORES + 1)) --export=ALL,shared_variables_file="${shared_variables_file}" -o "$job3_out" -e "$job3_err" "$tree_inference_script")
fi
